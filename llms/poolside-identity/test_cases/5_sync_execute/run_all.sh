...
echo "=== Test Case 5.3: Execute dry-run after initial sync ==="
echo "Running: $POOLSIDE_ID --env sandbox sync team1 $PROJECT_ROOT/team1.csv (verify idempotent dry-run)"
echo "Expected: Shows no changes needed (idempotent behavior)"
$POOLSIDE_ID --env sandbox sync team1 "$PROJECT_ROOT/team1.csv" 2>&1 || echo "Test 5.3: Note - dry-run may show updates"
